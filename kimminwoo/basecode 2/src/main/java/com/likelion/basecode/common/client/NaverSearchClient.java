...
public class NaverSearchClient {
    ...
    public List<NaverResponseDto> search(String query) {
        // url 만들기
        URI uri = UriComponentsBuilder.fromUriString(baseUrl + "/blog.json") // 네이버블로그는 blog.json으로 url 시작
                .queryParam("query", query)       // 검색어
                .queryParam("display", 3)         // 몇 개 보여줄지
                .queryParam("start", 1)           // 몇 번째부터 시작할지
                .build()
                .encode()
                .toUri();
    ...
    private NaverResponseDto toDto(Map<String, Object> item) {
        return new NaverResponseDto(
                (String) item.getOrDefault("title", ""),         // 제목
                (String) item.getOrDefault("link", ""),          // 링크
                (String) item.getOrDefault("description", "")    // 요약
        );
    }
